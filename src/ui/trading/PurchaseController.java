...
public class PurchaseController implements Initializable{
		...
		if(addValidationCheck()) {
			...
			for(rowCount = 0;rowCount < listProductDetails.size();rowCount++) {
				...
				if(tempPd.getItemId().equals(itemId)) {
						...
						listProductDetails.add(rowCount,new ProductDetails(itemId, getCmbItemName(), unit, unitQuantity, quantity, unitPrice, unitPrice, totalAmount, discount, netAmount));
			...
			if(rowCount == listProductDetails.size()) {
					...
					listProductDetails.add(new ProductDetails(itemId, getCmbItemName(), unit, unitQuantity, quantity, unitPrice, unitPrice, totalAmount, discount, netAmount));
					clearTxtAfterItemAdd();
		...
		try {
			if(!getCmbFind().isEmpty()) {
				if(isInvoiceIdExist(getCmbFind())) {
					...
					rs = databaseHandler.execQuery(sql);
					while(rs.next()) {
						listProductDetails.add(new ProductDetails(rs.getString("itemId"),rs.getString("itemName"), rs.getString("unit"), rs.getDouble("UnitQty"), rs.getDouble("qty"), rs.getDouble("unitPrice"), rs.getDouble("PurchasePrice"), rs.getDouble("totalamount"), rs.getDouble("discount"), rs.getDouble("netAmount")));
					}
	...
	public static class ProductDetails{
		...
		public ProductDetails(String itemId,String itemName,String unit,double unitQuantity,double quantity,double unitPrice,double price,double totalAmount,double discount,double netAmount) {
